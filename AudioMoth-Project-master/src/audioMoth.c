...
#define AM_USB_MSG_TYPE_SET_STATION_ID            0x0B
#define AM_USB_MSG_TYPE_GET_STATION_ID            0x0C
...
/*static bool writeToTextFile(const uint8_t *buffer, uint32_t length) {
    FILE *file = fopen(FILENAME, "a"); // Cambiamos "w" por "a"
    if (!file) {
        perror("Error opening file");
        return false;
    }

    size_t bytesWritten = fwrite(buffer, sizeof(uint8_t), length, file);
    fclose(file);
...
void handleUSBPacket() {
    ...
    switch(receivedMessageType) {
        ...
        case AM_USB_MSG_TYPE_GET_STATION_ID:
            /* Respond with the station ID */
            transmitBuffer[1] = stationId;
            break;
